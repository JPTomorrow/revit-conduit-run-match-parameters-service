...
namespace JPMorrow.Revit.Tools.ConduitFittings
{
	...
	readonly public struct Fitting
	{
		...
		public string GetAngleString(ModelInfo info) => UnitFormatUtils.Format(info.DOC.GetUnits(), UnitType.UT_Angle, Angle, true, false, CustomFormatValue.FeetAndInches);
			...
			foreach(var fitting in fittings)
			{
				...
				x.Fitting.Type.Equals(fitting.Type)))
				{
					var old_entry = ret_fc.Find(
						x => (x.Fitting.Angle >= min_angle_tolerance && x.Fitting.Angle <= max_angle_tolerance) &&
						x.Fitting.Diameter.Equals(fitting.Diameter) &&
						x.Fitting.Type.Equals(fitting.Type));
